...
export const generateFonepayQR = (amountNPR: number, orderId: string): FonepayQRData => {
  ...
  const qrPayload = `00020101021226500010np.fonepay0113PUJA-COLLECT02109811313666520456915303524540${amountNPR.toString().length}${amountNPR}5802NP5922PUJA COLLECTION PVT6007RANGELI62${(orderId.length + 4).toString().padStart(2, '0')}01${orderId.length.toString().padStart(2, '0')}${orderId}6304`;
