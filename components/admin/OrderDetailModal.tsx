...
export function OrderDetailModal({ order, onClose }: OrderDetailModalProps) {
  ...
  const handleRemoveItem = async (itemId: string, itemName: string) => {
    ...
    setIsSubmitting(true);
    setActionError('');
    const res = await removeItemFromOrder(liveOrder.orderId, itemId);
    setIsSubmitting(false);
  ...
  const handleCancelOrder = async () => {
    if (
      !window.confirm(
        `Are you sure you want to CANCEL Order #${liveOrder.orderId}?\n\nThis will mark the order as Cancelled and automatically restore product stock.`
      )
    ) {
      return;
    }
  ...
  const handleDeleteOrder = async () => {
    if (
      !window.confirm(
        `⚠️ PERMANENT ACTION:\n\nAre you sure you want to permanently DELETE Order #${liveOrder.orderId}?\nThis cannot be undone.`
      )
    ) {
      return;
    }
